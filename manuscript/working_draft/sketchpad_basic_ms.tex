...
To explain these findings, we developed a computational model of visual communciation that embodied two core competencies: visual abstraction, the capacity to perceive the correspondence between a real-world object and a simple drawing of it; and pragmatic inference, the ability to infer what information is not only \textit{valid} to include in a drawing, but also \textit{relevant} in context  \cite{goodman2016pragmatic,grice1975syntax,abell2009canny}.
...
% \rdh{Maybe make this panel A, since it comes first in the text, and use `diagnosticity' instead of `informativity' in the figure titles to match the terminology here}
