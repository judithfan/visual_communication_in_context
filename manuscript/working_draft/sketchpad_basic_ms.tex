...
For present purposes, we this problem by assuming that the sketcher model chooses among a large but finite set of sketches: those actually produced by participants in our task.
...
This sketcher judges a sketch's informativity to be a mixture of two quantities: one reflecting its absolute \textit{resemblance} to the target and the other its relative \textit{diagnosticity} in the context of particular distractors (see Fig. \ref{model_schematic}B).  \rdh{Maybe make this panel A, since it comes first in the text, and use `diagnosticity' instead of `informativity' in the figure titles to match the terminology here}
A sketch's resemblance is defined by the absolute degree of perceptual correspondence between a sketch $s$ and the target $t$, i.e. $\textrm{sim}(s,t)$. 
Our approach to estimating this correspondence for each sketch-object pair is provided in the following section. 
...
Finally, we define a sketch's cost, $C(s)$, to be a monotonic function of the empirical amount of time taken to produce it, linearly transformed  to lie in the range $[0,1]$. % via min-max normalization \ndg{should i know what "min-max normalization" is? it sounds really fancy...}
...
\rdh{Say exactly what layers we're using somewhere?}
