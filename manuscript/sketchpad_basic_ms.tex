...
Integrating these insights with prior work \cite{FanCommon2018,goodman2016pragmatic,grice1975syntax} on visual production and pragmatic inference during communication, we propose that human sketchers determine what kind of sketch to produce in context by deploying three main faculties: (a) visual abstraction, the ability to judge how well a simple sketch evokes a real object; (b) context sensitivity, a preference for sketches that are diagnostic of the target relative to the distractors; and (c) cost sensitivity, a preference for less costly sketches.
To test this proposal, we developed a computational model of the sketcher that embodied these three faculties. 
Constructing such a model allowed us to evaluate the independent contribution of each faculty using formal model comparison, and characterize the model's behavior in novel contexts.
...
In our full model of the sketcher, $\mathcal{S}$, we define the informativity term to be a mixture of the purely diagnosticity-based definition employed by $S_{2C}$ and the purely resemblance-based definition employed by $S_{sim}$. 
...
In order for this sketcher to generate quantitative predictions, it needs to be able to compute $\textrm{sim}(s,o)$, the perceptual correspondence, for any sketch-object pair. We approached this problem in two ways: by using human responses on the sketch-object matching task as an empirical approximation to $\textrm{sim}(s,o)$ for every sketch in our dataset, and training a visual encoder model to predict $\textrm{sim}(s,o)$ for any sketch-object pair in our dataset, once trained on empirical estimates of $\textrm{sim}(s,o)$ for a separate subset of our data. 
...
This choice of model class is further motivated by prior work showing that such networks, when optimized to recognize objects in the highly variable photographs in the ImageNet dataset \cite{deng2009imagenet}, learn higher-layer feature representations that capture high-level perceptual information in drawings \cite{FanCommon2018}, capture perceptual judgments of object shape similarity \cite{kubilius2016deep}, and predict neural population responses in categories across the ventral visual stream \cite{yamins2014performance}. 
Consistent with this prior work, we hypothesized that the best-fitting visual encoder would take advantage of the more abstract visual feature representation in higher layers of deep convolutional neural networks pre-trained on natural object recognition tasks, rather than the lower-level image statistics computed in earlier layers.

Using the empirical estimates $\textrm{sim}(s,o)$ 
...
Formally, the visual encoder is instantiated as a deep convolutional neural network (DCNN), a member of the class of universal function approximators \cite{hornik1991approximation}. This choice of model class is further motivated by prior work showing that such networks, when optimized to recognize objects in the highly variable photographs in the ImageNet dataset \cite{deng2009imagenet}, learn feature representations that capture adult perceptual judgments of object shape similarity \cite{kubilius2016deep} and predict neural population responses in categories throughout object-selective cortex \cite{yamins2014performance}. 
The encoder accepts a pair of images, a sketch, $s$, and an object rendering, $o$, as input and returns a scalar value reflecting the degree of perceptual correspondence between the sketch and object, $\textrm{sim}(s,o)$, which lies in the range $[0,1]$, where $0$ reflects no correspondence and $1$ reflects maximal correspondence (Eq. \ref{visual_encoder}).
...
This module is composed of two functions: a base visual encoder network, $b$, and an adaptor network, $a$. We employ VGG-19 \cite{simonyan2014very} pretrained on Imagenet as our base visual encoder, whose parameters do not undergo further fine-tuning. 
...
We use this adaptor network because while prior work has shown that the representation of object categories converges for sketches and photos at higher layers in pretrained DCNN models \cite{FanCommon2018}, learning additional transformations can greatly improve the accuracy of comparisons between specific images across modalities \cite{sangkloy2016sketchy}. 
