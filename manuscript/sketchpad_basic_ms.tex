...
Because the goal of our modeling experiments was to ascertain how well each model could modulate its behavior according to the context condition, and not necessarily to reproduce exactly the same sketch as a particular participant had on a specific trial, we aggregated sketches of the same object produced in the same context condition when estimating their key functional properties (i.e., perceptual correspondence, cost). Model predictions were generated to the same level of granularity, yielding a probability distribution over each of 64 categories of sketches, reflecting each combination of object-context conditions (e.g., `basset sketch produced in a close context'). 

To generate predictions, first we employed Bayesian data analysis to infer a posterior distribution over the four latent scaling parameters applied to the informativity ($w_{i}$), cost ($w_{c}$), diagnosticity ($w_{d}$), and $\textrm{sim}(s,o)$ ($\alpha$) terms. 

Next, we presented the model with exactly the same set of contexts as were presented to human sketchers in the communication experiment, and evaluated the probabilities assigned to sketches from each object-context category, marginalizing over the joint posterior distribution over latent parameters. 

We found that the pragmatic sketcher, $S_{2C}$, was the only of these three model variants to 
