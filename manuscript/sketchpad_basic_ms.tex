...
We define our full sketcher, $\mathcal{S}$, to be a decision-theoretic agent that evaluates sketches, $s$, of object referent, $o$, proportional to their relative communicative utility, $U$ (Eq. \ref{sketcher_distribution}), where each sketch's absolute communicative utility has been normalized via the softmax function:

\begin{equation} \label{sketcher_distribution}
\mathcal{S}(s|o) \propto \exp \{{U(s,o)\}}
\end{equation}

The utility function of the full context-sensitive and cost-sensitive sketcher, $U_{S}$, trades off the extent to which a given sketch is informative to an imagined viewer, $\mathcal{V}$, with the cost of producing that sketch, $C(s)$. This notion of informativity is defined by the natural log probability that a viewer would select the true object given the sketch and all objects in context. In our implementation, the cost of a sketch is operationalized as the amount of time taken to produce it, though other metrics could be used (e.g., the number of strokes, proportion of canvas filled).

\begin{equation} \label{sketcher_utility}
U_{S}(s, o) = w_p \ln \mathcal{V}(o|s) - w_c C(s)
\end{equation}
