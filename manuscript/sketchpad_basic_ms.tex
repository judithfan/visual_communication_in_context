...
Experimental data were preprocessed in various ways before being 
...
% This human response distribution 
...
To evaluate the degree to which the \textit{visual abstraction} afforded by the complex transformations applied by successive layers of VGG-19 are required to capture human behavior during visual communication, we construct three adaptor networks that intercept VGG-19 image representations at different layers: the first max pooling layer (early), the tenth convolutional layer (intermediate), and the first fully connected layer (high). 

To facilitate comparison between adaptor networks, we ensured that they contained a comparable number of trainable parameters (Table~\ref{table:parameters}), and were trained under matched conditions. 

To help discriminate which layer provides the best starting features basis for predicting sketch-object correspondence, these adaptor networks were also deliberately constrained to be shallow relative to the depth of VGG-19 \todo{(i.e., consisting of two layers including nonlinearities? what is best way to say this?)}.

In the following, we provide a detailed description of the architecture and hyperparameter choices for each adaptor. We refer to each adaptor network as: \textit{adaptor-early}, \textit{adaptor-mid}, and \textit{adaptor-high}. 
...
Adaptor-high is composed of a linear layer from 8192 to 128, followed by a swish nonlinearity \cite[]{ramachandran2018searching} \todo{brief intuition for what swish does goes here} and dropout, then a linear layer from 128 to 1. Each vgg-high feature has 4096 dimensions. A photo and sketch feature is first concatenated to form a vector of size 8192, then passed to adaptor-high. In practice, we find the ability of the model to fit the data is robust to the choice of nonlinearity \todo{alternative nonlinearities were X, Y, Z} and size of hidden layer, within the range $(2^{7},2^{9})$. 

\todo{Has the adaptor-mid architecture changed since this writing?}
% https://github.com/judithfan/pix2svg/blob/master/generative/tests/compare_test/model.py
...
%% Is this the line where the adaptor arch. is defined? https://github.com/judithfan/pix2svg/blob/master/generative/tests/compare_test/model.py#L81
Finally, adaptor-low is almost identical to adaptor-mid, except each feature is of size 64 by 112 by 112. Explicitly, this network starts with a linear layer of 4096 to 7875 \todo{Trying to understand why 4096->7875? Does it not start with a pooling layer, followed by channel-wise attention? Is this the same as the mid-adaptor?}, swish, dropout \todo{how much dropout?}, then a linear layer form 7875 to 1. Due to the input size, each attention module has 12,544 learnable weights. 

%% adaptor training procedure
We now describe the training procedure for each visual encoder. The sketch dataset is split into training, validation, and test sets in a 80\%, 10\%, and 10\% ratio, and this split was performed in a 5-fold cross-validated manner. This division is performed based on context, which is defined as the set of the target and distractor images (each trial consists of 3 distractor images and 1 target). Doing so, the contexts of training examples will not overlap with the contexts of either validation or test examples. \todo{I recall these were also balanced by close/far, were there any other balancing dimensions?} Given a photo and a sketch, the adaptor network outputs a single real number, which we use as a similarity measure. We construct a similarity vector of size 32 between a sketch and each object rendering in the domain (32 total), and apply a softmax on top to induce a probability distribution over objects. To fit network parameters, we optimize via stochastic gradient descent on the cross entropy loss between the induced distribution and a distribution of average human recognition responses. \todo{write out cross entropy loss fn explicitly} The human response distribution was derived by computing the proportion of trials from the recognition experiment on which a sketch from the same context-object category (e.g., all sparrow sketches produced on close trials) was matched with each object. As a consequence, this human response distribution not only provides an estimate of the strength of the match between a sketch and its corresponding object, but also the pattern of confusions that human observers exhibit, thus providing a more detailed profile of recognition behavior for each adaptor to emulate. We use the Adam optimization algorithm \cite[]{kingma2014adam} with a learning rate of 1e-4 and minibatch size of 10 for 100 epochs. As a property of the input domain, the gradients with respect to adaptor parameters are very small \todo{how small? in the range Xe-X to Xe-X}. We find that increasing the learning rate alone led to unstable optimization, but that multiplying the loss by a large constant $C$ leads to optimize the objective for good test performance. In practice, $C = 1e4$. After training each adaptor for 100 epochs, we use the model with the best performance on the validation set and freeze its weights for evaluation. For each photo and sketch pair in the test set, we evaluate the perceptual correspondence and provide this as input to the social reasoning module, described below.

\subsubsection*{Social reasoning and decision module}
...
% All model comparison was done in a crossvalidated manner on data from the test set of each crossvalidation split.
