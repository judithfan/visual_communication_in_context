...
Rather, these results suggest that semantic context plays a critical role in determining how informative and costly of a sketch people produce during visual communication.
And they suggest an analogy to how semantic context influences how people produce linguistic utterances during verbal communication, a key aspect of pragmatic competence \cite{grice1975syntax,goodman2016pragmatic}. 
Bringing together insights from prior work on visual production and pragmatics \cite{FanCommon2018,goodman2016pragmatic}, we propose that human sketchers determine what kind of sketch to produce in context by deploying two faculties: (a) visual abstraction, the ability to judge how well a simple sketch evokes a real object, and (b) pragmatic inference, a preference to produce sketches that are sufficiently diagnostic of the target object in context, but no more diagnostic --- and thus, costly --- than necessary. 
Concretely, this latter faculty encompasses two aspects: context sensitivity, a preference for sketches that are diagnostic of the target relative to the distractors; and cost sensitivity, a preference for less costly sketches.
To test this proposal, we developed a computational model of the sketcher that embodied these two faculties, and was instantiated as a probabilistic program wrapped around a deep convolutional neural network. 
Constructing such a model allowed us to evaluate the independent contribution of each faculty using formal model comparison, and quantitatively characterize the model's behavior in novel contexts.

We define the sketcher $\mathcal{S}$, to be a decision-theoretic agent that prefers to produce a sketch, $s_i$, of target object, $t$, in a context containing distractors, $d$, proportional to their communicative utility, $U$, where the utilities of each sketch have been normalized over the space of producible sketches via the softmax function (Eq. \ref{sketcher_distribution}):

\begin{equation} \label{sketcher_distribution}
\mathcal{S}(s_i|t, d) \propto \frac {\exp [{U(s_i,t,d)]}} {\sum_{j=1}^{N} {\exp [U(s_j,t,d)]}}
\end{equation}
...
The imagined viewer $\mathcal{V}$, in turn, is assumed to select an object in context proportional to the absolute perceptual correspondence between each object and the current sketch, $\textrm{sim}(s,o)$.

\begin{equation} \label{literal_viewer_score}
\mathcal{V}(t|s) \propto \exp\{\alpha \cdot \textrm{sim(s, o)}\}
\end{equation}
where $\alpha$ is another scaling parameter determining the assumed optimality of the listener's decision policy: as $\alpha \rightarrow \infty$, the imagined listener is more likely to choose the object with highest perceptual correspondence to the sketch.

Second, we define the utility function for a \textit{context-insensitive} sketcher, $S_{sim}$, which replaces the diagnosticity term (defined in terms of the viewer) with a resemblance term reflecting the absolute degree of perceptual correspondence between a sketch and the target, $\textrm{sim}(s,o)$, ignoring the distractors. 
...
To do this, we infer a mixture weight parameter, $w_{diag}$, that interpolates between these two notions of informativity: $$f(s,o) = w_{diag}\ln \mathcal{V}(o | s) + (1-w_{diag}) \textrm{sim}(s,o)$$. 
...
In order for this sketcher to generate quantitative predictions, it needs to be able to compute the perceptual correspondence ($\textrm{sim}(s,o)$) for any sketch-object pair. We approached this problem in two ways: by employing human responses on the sketch-object matching task as an empirical approximation to $\textrm{sim}(s,o)$ for every sketch in our dataset, and employing a visual encoder model to predict $\textrm{sim}(s,o)$, once trained on empirical estimates of $\textrm{sim}(s,o)$ for a separate subset of our data. 

Informed by recent advances in computational vision \cite{FanCommon2018,yamins2014performance}, we instantiated the visual encoder as a deep convolutional neural network (DCNN), a member of the class of universal function approximators \cite{hornik1991approximation}. 
This choice of model class is further motivated by prior work showing that such networks, when trained on challenging natural object recognition tasks \cite{deng2009imagenet}, learn higher-layer feature representations that capture high-level perceptual information in drawings \cite{FanCommon2018}, capture perceptual judgments of object shape similarity \cite{kubilius2016deep}, and predict neural population responses in categories across the ventral visual stream \cite{yamins2014performance}. 
...
Concretely, the visual encoder is a function that accepts a pair of images, a sketch, $s$, and an object rendering, $o$, as input and returns a scalar value reflecting the degree of perceptual correspondence between the sketch and object, $\textrm{sim}(s,o)$, which lies in the range $[0,1]$, where $0$ reflects no correspondence and $1$ reflects maximal correspondence (Eq. \ref{visual_encoder}).
...
The encoder consists of two functional components: a base visual encoder network, $b$, and an adaptor network, $a$. We employ VGG-19 \cite{simonyan2014very} pretrained on Imagenet as our base visual encoder, whose parameters do not undergo further fine-tuning. 
...
We train an adaptor network because while prior work has shown that the representation of object categories converges for sketches and photos at higher layers in DCNN models trained only on photos \cite{FanCommon2018}, additional supervision can substantially improve the accuracy of image-level predictions involving comparisons between sketches and photos \cite{sangkloy2016sketchy}. 
...
Within this modeling framework, we conducted series of targeted lesion studies to test each aspect of our hypothesis about the core cognitive faculties employed by humans to decide which sketch to produce in context. First, we evaluated the contribution of context sensitivity and cost sensitivity by removing each of these faculties from the sketcher agent; these experiments relied upon empirical estimates of $\textrm{sim}(s,o)$. Second, we replicated these lesion experiments with the visual encoder model, which allowed us to further evaluate the contribution of using high-level visual features, in addition to cost and context sensitivity.

\subsubsection*{Evaluating context sensitivity}

\subsubsection*{Evaluating cost sensitivity}
...
\subsubsection*{Evaluating visual abstraction}
