...
Within this modeling framework, we conducted series of targeted lesion studies to test each aspect of our hypothesis about the core cognitive faculties employed by humans to decide which sketch to produce in context. 
First, we evaluated the contribution of pragmatic inference by removing context sensitivity and cost sensitivity, in turn, from the sketcher agent; these experiments relied upon empirical estimates of $\textrm{sim}(s,o)$. 
Second, we replicated these lesion experiments after incorporating the visual encoder, which allowed us to further evaluate the contribution of using high-level visual features, in addition to pragmatic inference.
