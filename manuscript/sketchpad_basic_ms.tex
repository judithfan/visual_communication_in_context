...
We explored the hypothesis that people spontaneously take information in common ground with their communication partner into account to produce drawings that are diagnostic of the target relative to the alternatives, while not being too costly to produce. 
We found that people spontaneously modulate how much time they invest in their drawings according to how similar the distractors are to the target, spending more time to produce more informative drawings when the alternatives were highly similar, but getting away spending less time and producing less informative drawings when the alternatives were highly distinct.
Observing such contextual flexibility argues against the notion that visual production is constrained exclusively by the perceptual properties of the target. 
Rather, they led us to hypothesize that two abilities are critical to explain human contextual flexibility in this task: visual abstraction, the capacity to perceive the correspondence between an object and a drawing of it; and pragmatic inference, the ability to infer what information would be relevant for helping a viewer distinguish the target from the distracotrs. 
To test this hypothesis, we developed a computational model that embodied both visual abstraction and pragmatic inference, instantiated as a probabilistic program ``wrapped'' around a deep convolutional neural network, and found that it fit the data well and outperformed lesioned variants of the model. 


% related literatures
	% relationship to RSA models to language
	% other graphical communication papers

% limitations of this work
	% pix2svg -- for image-level predictions
