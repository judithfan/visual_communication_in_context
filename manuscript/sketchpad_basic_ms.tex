...
\subsection*{Computational model of communicative sketching}

%% describe the full sketcher model
To make fine-grained quantitative predictions and conduct a rigorous comparison, we formalized our hypotheses as a series of computational models. We consider three key components that must be integrated in a sketching agent: \emph{pragmatics}, \emph{perception}, and \emph{production} (see Fig. \ref{fig:modelSchematic}). \emph{Pragmatics} determines whether the sketcher reasons about how their partner will make decisions in context. \emph{Perception} determines the encoding model used by the agent to determine the literal similarity between a drawing and object. \emph{Production} determines whether or not the agent accounts for the relative costs of drawing. We conduct a set of comparisons against lesioned variants to test the role played by each component. 

%% 4 versions: prag with cost, prag no cost, no prag cost, no prag no cost
We test the pragmatic component by comparing a \emph{social sketcher}, which takes context into account, against an \emph{asocial sketcher} that simply aims to make a sketch with high absolute perceptual similarity to the target, ignoring context. To define our social sketcher, we generalize the Rational Speech Act (RSA) framework, which models language understanding as recursive social reasoning (\emph{Materials and Methods}; Eq. 2-4). By reasoning about a viewer who normalizes perceptual similarity across objects in context, a social sketcher chooses sketches that are informative, or that apply to the target better than distractors. For example, in a context only containing birds, a generic outline of a bird would not be very informative, even if it has high absolute similarity to the target, because it also resembles the distractors.

For the perceptual component, we compare similarity values directly measured from human raters in our recognition task against three similarity functions derived from neural network models. In particular, we 

Finally, to test production, we simply lesion the cost term from the speaker's utility function. 
...
Formally, our sketcher, $\mathcal{S}$, is a decision-theoretic agent who produces sketches, $s$, by (soft)-maximizing their utility function, $U$, given a particular object referent, $o$:

\begin{equation}
\mathcal{S}(s|o) \propto \exp\{{U(s,o)\}}
\end{equation}

The sketcher's utility function, $U$, trades off the extent to which the sketch is informative to the viewer, $\mathcal{V}$, and the cost of producing the sketch, $C(s)$. This notion of informativity is defined by the (natural log) probability that a viewer would select the true object referent given the drawing and current context:
\begin{equation} \label{sketcher_utility}
U(s, o) = w_1 \ln \mathcal{V}(o|s) - w_2 C(s)
\end{equation}
where $w_1$ and $w_2$ are inferred parameters weighting the influences of the informativity and cost terms, respectively. 
...
\mathcal{V}(o|s) \propto \alpha \cdot \textrm{sim}(s, o)
\end{equation}
