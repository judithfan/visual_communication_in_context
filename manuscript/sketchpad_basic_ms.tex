...
To evaluate this hypothesis, we developed a computational model of the sketcher that embodied both competencies, instantiated as a a deep convolutional neural network nested within a probabilistic program. 
...
To test this proposal, we developed a computational model of the sketcher that embodied both visual abstraction and pragmatic inference, and was instantiated as a a deep convolutional neural network nested within a probabilistic program. 
...
\caption{Bayes Factors for model comparisons between full and lesioned model variants (columns) for each crossvalidation fold (rows). Results under `human recog' heading (first two columns) reflect model predictions when using empirical estimates of perceptual correspondence based on human sketch recognition behavior. Results under `visual encoder' heading (final four columns) reflect model predictions when using variants of the DCNN visual encoder, the adaptor components of which were trained in a five-fold crossvalidated manner using human sketch recognition behavior. Other columns indicate comparisons between full model and different types of lesion: `context' reflects comparison to context-insensitive variant; `cost' reflects comparison to cost-insensitive variant; `mid' reflects comparison between full model using the high adaptor vs. mid adaptor; `low' reflects comparison between full model using high adaptor vs. low adaptor.}
...
% \mwu{is there any significance to these choices? why did we choose these?}
...
% \mwu{im lost; is this describing how pairs were chosen or what the viewer sees? what is a quartet?} 
