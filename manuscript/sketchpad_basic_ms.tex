...
\significancestatement{Drawing is a versatile tool for communication, from detailed circuit diagrams to casual whiteboard sketches. Even the same object can be drawn in many ways, depending on the context. How do people decide what to draw in order to be understood? We collected a large number of drawings in different contexts and found that people adapted their drawings accordingly, producing detailed drawings when necessary, but simpler drawings when sufficient. To explain this contextual flexibility, we developed a computational model that combines the capacity for visual abstraction (perceiving the correspondence between an object and drawing) with social reasoning (inferring what information is relevant to the viewer in context). Our results suggest drawing is so versatile because people embody both capacities.}
