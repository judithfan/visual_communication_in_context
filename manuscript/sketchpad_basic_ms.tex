...
We define our full sketcher, $\mathcal{S}$, to be a decision-theoretic agent that produces sketches, $s$, of target object, $t$, in a context containing distractors, $d$, proportional to their communicative utility, $U$, where each sketch's utility has been normalized via the softmax function (Eq. \ref{sketcher_distribution}):
...
The utility function of this sketcher, $U_{S}$, consists of two terms: an informativity and a cost term. A sketch's informativity is defined as the natural log probability that an imagined viewer, $\mathcal{V}$, would select the target object given the sketch and all objects in context, $\ln \mathcal{V}(t|s,d)$. A sketch's cost is defined to be a monotonic function of the amount of time taken to produce it $C(s)$. 

\begin{equation} \label{sketcher_utility}
U_{S}(s, o) = w_i \ln \mathcal{V}(o|s) - w_c C(s)
\end{equation}
...
\end{equation}
where $\alpha$ is another scaling parameter determining the assumed optimality of the listener's decision policy: as $\alpha \rightarrow \infty$, the listener is more likely to choose the object with highest perceptual correspondence to the sketch.
