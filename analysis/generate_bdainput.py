...
def generate_bdaInput_csv(D,filtration_level,train_test_split=True,
    ...
    if 'multimodal' in args.adaptor_type:
        ...
        adaptor = 'human'
    test_examples = pd.read_json('./test_examples/{}_{}_{}_test_examples.json'.format(args.iterationName,adaptor,args.split_type),orient='records')
...
if __name__ == "__main__":
    ...
    if ('human' not in args.adaptor_type):
        ...
        for obj in obj_list:
            ...
            ## 5/30/18: get indices of invalid sketches from the similarities dictionary
    ...
    if not os.path.exists(full_out_path):
        ...
        aa = D2['normed_cost_strokes'].values
        bb = __D2['normed_cost_strokes'].values
        if np.sum(aa-bb)>1e-8:
