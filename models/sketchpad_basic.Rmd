...
```{r}
pragSimilarities <- fromJSON('./refModule/json/similarity-splitbycontext.json', flatten = T)
...
d.similarity %>%
  ...
  group_by(coarseGrainedSketchInfo) %>%
  summarize(m = mean(k)) %>%
  ungroup() %>%
  summarize(sum(m))
```
...
errorsByDomain %>% 
  mutate(withinDomain = ifelse(trueDomainLabel == possibleDomain, 'within', 'across')) %>%
...
```{r}
ggplot(errorsByObj %>% mutate(prob = ifelse(prob < -4, -4, prob)), 
       aes(x = trueSketch, y = desc(possibleSketch), fill = prob)) +
  geom_tile() + 
  geom_vline(xintercept = seq(8.5, 8.5*10, 8)) +
